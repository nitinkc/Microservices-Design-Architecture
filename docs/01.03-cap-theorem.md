...
!!! warning  A network partition, 
         or split-brain scenario, occurs when a distributed system's network breaks, dividing nodes into isolated subnets that cannot communicate with each other.
...
| **Zookeeper**                             | Majority **_quorum_** required for reads/writes              |
